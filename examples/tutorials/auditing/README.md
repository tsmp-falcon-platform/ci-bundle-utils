...
You can start auditing your entire environment simply with:

- a container running the `bundleutils` tool
- a username/token
- a repository and git credentials for committing
...
- If not set, the config file packaged with this image is used
  - The file used is here: [.gitleaks.toml](../auditing/.gitleaks.toml)
...
The [./audit.sh](./audit.sh) file needs to know what to do when:
